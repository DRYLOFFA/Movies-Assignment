...
namespace Movies_Project
{
    ...
    {
        SqlConnection SqlConn = new SqlConnection("Data Source=SILVERDRAGON-PC\\SqlExpress01;Initial Catalog=Movies_Project;Integrated Security=True");
        ...
        public DataTable Stats()
        {
            ...
            {
                SqlStr.Connection = SqlConn;
                SqlStmt = "select count (Title) as Times_Rented, Title as Title from RentedMovies r INNER JOIN Movies m on r.MovieIDFK = m.MovieID" +
                " group by Title" +
        ...
        public DataTable BestCustomer()
        {
            ...
            {
                SqlStr.Connection = SqlConn;
                SqlStmt = "Select count(FirstName) as Movies_Rented, FirstName, LastName" +
                " from Customer c INNER JOIN RentedMovies rm on c.CustID = rm.CustIDFK INNER JOIN Movies" +
                " m on m.MovieID = rm.MovieIDFK where CustID = CustID" +
                " group by FirstName, LastName order by Movies_Rented desc";
